...
left join (select max(Salary) as maxSalary, DepartmentId from Employee group by DepartmentId) ne
on e.Salary = ne.maxSalary
where ne.maxSalary is not null and d.Name is not null